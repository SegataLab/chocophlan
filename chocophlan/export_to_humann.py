...
def initialize (config):
    ...
    os.makedirs('{}/{}/functional_annot/'.format(shared_variables.config['export_dir'], shared_variables.config['exportpath_humann']), exist_ok=True)
    os.makedirs('{}/{}/panproteomes_fna/'.format(shared_variables.config['export_dir'], shared_variables.config['exportpath_humann']), exist_ok=True)
    os.makedirs('{}/{}/gca_upkb_to_nr/'.format(shared_variables.config['export_dir'], shared_variables.config['exportpath_humann']), exist_ok=True)
...
def export_panproteome_centroid(species_id):
    ...
    # with mpdummy.Pool(processes=nproc) as pool_t:
...
def export_panphlan_panproteome(species_id):
    ...
    with mpdummy.Pool(initializer = init_parse, initargs = (terminating, ), processes=100) as pool:
    ...
    if len(gc):
        ...
        terminating = mpdummy.Event()
        with mpdummy.Pool(initializer = init_parse, initargs = (terminating, ), processes=100) as pool_t:
...
def export_genome_annotation(panproteome_id):
    ...
    if gca_id and len(shared_variables.proteomes[panproteome_id]['members']):
        ...
        with bz2.open('{}/{}/gca_upkb_to_nr/{}.txt.bz2'.format(shared_variables.config['export_dir'], shared_variables.config['exportpath_humann'], gca_id), 'wt') as fasta_pangenes:
