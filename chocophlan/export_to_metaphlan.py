...
from chocophlan._version import __CHOCOPhlAn_version__
from chocophlan._version import __UniRef_version__
from chocophlan._version import __MetaPhlAn_db_version__
import chocophlan.utils as utils
from chocophlan.panproteomes import Panproteome
...
class export_to_metaphlan2:
    def __init__(self, config):
        ...
        self.exportpath = '{}/{}'.format(config['export_dir'], config['exportpath_metaphlan'])
    ...
    def get_uniref_uniprotkb_from_panproteome(self, panproteome):
        ...
        if len(markers_nucls) > 9:
            with bz2.open('{}/{}/markers_fasta/{}.fna.bz2'.format(self.config['export_dir'], self.config['exportpath_metaphlan'], panproteome['tax_id']), 'wt') as fasta_markers:
        ...
        if len(gc):
            pickle.dump(gc, open('{}/{}/markers_info/{}.txt'.format(self.config['export_dir'], self.config['exportpath_metaphlan'], panproteome['tax_id']), 'wb'))
        possible_markers.to_csv('{}/{}/markers_stats/{}.txt'.format(self.config['export_dir'], self.config['exportpath_metaphlan'], panproteome['tax_id']))
    ...
    def merge_group(self, taxid):
        ...
        if len(gc):
            ...
            with mpdummy.Pool(processes=30) as pool:
        ...
        if len(markers_nucls) > 9:
            with bz2.open('{}/{}/markers_fasta/{}.fna.bz2'.format(self.config['export_dir'], self.config['exportpath_metaphlan'], merged_pangenome['tax_id']), 'wt') as fasta_markers:
                [fasta_markers.write(marker.format('fasta')) for marker in markers_nucls]
        elif len(markers_nucls) == 0:
            self.log.warning("[{}]\tNo nucleotide sequences extracted".format(taxid))
        if len(gc):
            pickle.dump(gc, open('{}/{}/markers_info/{}.txt'.format(self.config['export_dir'], self.config['exportpath_metaphlan'], merged_pangenome['tax_id']), 'wb'))

        possible_markers.to_csv('{}/{}/markers_stats/{}.txt'.format(self.config['export_dir'], self.config['exportpath_metaphlan'], merged_pangenome['tax_id']))
...
def merge_bad_species(sgb_release, gca2taxonomy, config):
    ...
    with open('{}/{}/merged_species_spp.tsv'.format(config['export_dir'],config['exportpath_metaphlan']), 'wt') as fout:
...
def brucella_markers(export):
    ...
    failed_uniprot_repr = []
    for np90_cluster in possible_markers.index:
        for (upkb_id, upid), count in merged_pangenome['members'][np90_cluster]['copy_number'].items():
            is_uparc = upkb_id.startswith('UPI')
    ...
    if len(gc):
        ...
        if len(failed_uniprot_repr) == possible_markers.shape[0]:
            ifnotgenome = not any([True for p in export.taxontree.get_child_proteomes(export.taxontree.taxid_n[235])
                                    if dict(export.proteomes[p].get('ncbi_ids',{})).get('GCSetAcc','')])
            export.log.warning('[{}]\tFailed to find any gene for all possible markers identified.{}'.format(merged_pangenome['tax_id'], "No assemblies are available for the species' proteomes" if ifnotgenome else ''))
        return (merged_pangenome['tax_id'], taxonomy)
    ...
    if failed is not None and not all(x is None for x in failed):
        failed = list(filter(None, failed))[0]
        possible_markers = possible_markers.loc[~possible_markers.index.isin(failed)]
        export.log.warning("{}: Failed to find features of markers {}".format(235, (','.join(failed))))

    if len(markers_nucls) > 9:
        with bz2.open('{}/{}/markers_fasta/{}.fna.bz2'.format(export.config['export_dir'], export.config['exportpath_metaphlan'], merged_pangenome['tax_id']), 'wt') as fasta_markers:
            [fasta_markers.write(marker.format('fasta')) for marker in markers_nucls]
    elif len(markers_nucls) == 0:
        export.log.warning("[{}]\tNo nucleotide sequences extracted".format(235))
    if len(gc):
        pickle.dump(gc, open('{}/{}/markers_info/{}.txt'.format(export.config['export_dir'], export.config['exportpath_metaphlan'], merged_pangenome['tax_id']), 'wb'))

    possible_markers.to_csv('{}/{}/markers_stats/{}.txt'.format(export.config['export_dir'], export.config['exportpath_metaphlan'], merged_pangenome['tax_id']))

    mpa_pkl = pickle.load(bz2.open('{}/{}/{}.pkl'.format(export.config['export_dir'], export.config['exportpath_metaphlan'].replace('_curated_markers',''), OUTFILE_PREFIX)))
    ...
    with bz2.open('{}/{}/{}.fna.bz2'.format(export.config['export_dir'], export.config['exportpath_metaphlan'], OUTFILE_PREFIX), 'wt') as outfile, \
            bz2.open('{}/{}/{}.fna.bz2'.format(export.config['export_dir'], export.config['exportpath_metaphlan'].replace('_curated_markers',''), OUTFILE_PREFIX), 'rt') as infile:
    ...
    with bz2.open('{}/{}/{}.pkl'.format(export.config['export_dir'], export.config['exportpath_metaphlan'], OUTFILE_PREFIX), 'wb') as pkl_out:
...
def run_all(config):
    ...
    if os.path.exists('{}/{}/{}'.format(config['export_dir'], config['exportpath_metaphlan'], OUTFILE_PREFIX)):
       if not os.path.exists('{}/{}/DONE'.format(config['export_dir'],config['exportpath_metaphlan'])):
           os.rename('{}/{}/{}'.format(config['export_dir'], config['exportpath_metaphlan'], OUTFILE_PREFIX), 
            '{}/{}/{}_bak_{}'.format(config['export_dir'], config['exportpath_metaphlan'], OUTFILE_PREFIX, datetime.datetime.today().strftime('%Y%m%d_%H%M')))
    
    for i in ['markers_stats','markers_fasta','markers_info']:
        os.makedirs('{}/{}/{}/{}'.format(config['export_dir'], config['exportpath_metaphlan'], OUTFILE_PREFIX, i))
    ...
    merged_species = pd.read_csv('{}/{}/merged_species_spp.tsv'.format(config['export_dir'],config['exportpath_metaphlan']), sep = '\t')
    export.species_merged_into = [int(x.split('|')[-1]) for x in set(merged_species.merged_into_id)]

    if not os.path.exists('{}/{}/DONE'.format(export.config['export_dir'],export.config['exportpath_metaphlan'])):
        ...
        with open('{}/{}/failed_species_markers.txt'.format(export.config['export_dir'], export.config['exportpath_metaphlan']), 'wt') as ofn_failed:
            ofn_failed.writelines('\n'.join('{}\t{}'.format(taxid, '\t'.join(taxonomy)) for taxid, taxonomy in filter(None, failed)))

        with mpdummy.Pool(processes=10) as pool:
        ...
        with bz2.BZ2File('{}/{}/{}.orig.pkl'.format(export.config['export_dir'], export.config['exportpath_metaphlan'], OUTFILE_PREFIX), 'w') as outfile:
            pickle.dump(export.db, outfile, protocol=2)
        utils.info('Done.\n')

        with open('{}/{}/DONE'.format(export.config['export_dir'],export.config['exportpath_metaphlan']), 'wt') as fcomp:
            fcomp.write(OUTFILE_PREFIX)

        utils.info('Merging all the markers sequences...\n')
        mpa_fna_db = '{}/{}/{}.fna'.format(export.config['export_dir'], export.config['exportpath_metaphlan'], OUTFILE_PREFIX)
        with open(mpa_fna_db, 'wb') as mpa2_markers_fna:
            for f in glob.glob('{}/{}/markers_fasta/*.fna.bz2'.format(export.config['export_dir'], export.config['exportpath_metaphlan'])):
        ...
        with bz2.open('{}/{}/{}.orig.pkl'.format(export.config['export_dir'], export.config['exportpath_metaphlan'], OUTFILE_PREFIX)) as infn:
    ...
    with open('{}/{}/{}_ext.tsv'.format(export.config['export_dir'], export.config['exportpath_metaphlan'], OUTFILE_PREFIX), 'w') as outfile:
        outfile.write('\n'.join('{}\t{}'.format(marker, ext_species['external_species']) for marker, ext_species in markers2ext.items()))

    with bz2.BZ2File('{}/{}/{}.orig.nomerged.pkl'.format(export.config['export_dir'], export.config['exportpath_metaphlan'], OUTFILE_PREFIX), 'w') as outfile:
    ...
    try:
        with bz2.open('{}/{}/{}.fna.bz2'.format(export.config['export_dir'], export.config['exportpath_metaphlan'], OUTFILE_PREFIX), 'wt') as outfile, \
             open('{}/{}/{}.fna'.format(export.config['export_dir'], export.config['exportpath_metaphlan'], OUTFILE_PREFIX), 'r') as infile:
            for record in SeqIO.parse(infile, 'fasta'):
                if record.id in mpa_pkl['markers']:
                    outfile.write(record.format('fasta'))
        with bz2.open('{}/{}/{}.fna.bz2'.format(export.config['export_dir'], export.config['exportpath_metaphlan'], OUTFILE_PREFIX), 'rt') as outfile:
            ids = [x[1:].split()[0].strip() for x in outfile if x[0] == '>']
        for marker in set(mpa_pkl['markers']).difference(ids):
            mpa_pkl['markers'].pop(marker)
                
    except:
        utils.remove_file('{}/{}/{}.fna.bz2'.format(export.config['export_dir'], export.config['exportpath_metaphlan'], OUTFILE_PREFIX))
        utils.error('Failed to compress the MetaPhlAn2 markers database. Exiting...')
        exit(1)
    finally:
        utils.remove_file('{}/{}/{}.fna'.format(export.config['export_dir'], export.config['exportpath_metaphlan'], OUTFILE_PREFIX))
    ...
    with open('{}/{}/merged_mapping'.format(export.config['export_dir'], export.config['exportpath_metaphlan']),'wt') as fout: 
    ...
    with bz2.BZ2File('{}/{}/{}.pkl'.format(export.config['export_dir'], export.config['exportpath_metaphlan'], OUTFILE_PREFIX), 'w') as outfile:
        pickle.dump(mpa_pkl, outfile, protocol=2)

    with bz2.open('{}/{}/{}_marker_info.txt.bz2'.format(export.config['export_dir'], export.config['exportpath_metaphlan'], OUTFILE_PREFIX), 'wt') as outfile:
        outfile.write('\n'.join('{}\t{}'.format(m, mpa_pkl['markers'][m]) for m in mpa_pkl['markers']))

    with tarfile.TarFile('{}/{}/{}.tar'.format(export.config['export_dir'], export.config['exportpath_metaphlan'], OUTFILE_PREFIX), 'w') as mpa_tar:
        mpa_tar.add(name = '{}/{}/{}.pkl'.format(export.config['export_dir'], export.config['exportpath_metaphlan'], OUTFILE_PREFIX),
                    arcname = '{}.pkl'.format(OUTFILE_PREFIX),
                    recursive = False )
                    
        mpa_tar.add(name = '{}/{}/{}.fna.bz2'.format(export.config['export_dir'], export.config['exportpath_metaphlan'], OUTFILE_PREFIX),
                    arcname = '{}.fna.bz2'.format(OUTFILE_PREFIX),
                    recursive = False)

    md5hash = hashlib.md5()
    with open('{}/{}/{}.tar'.format(export.config['export_dir'], export.config['exportpath_metaphlan'], OUTFILE_PREFIX),'rb') as f:
        for chunk in iter(lambda: f.read(4096), b""):
            md5hash.update(chunk)

    calc_hash = md5hash.hexdigest()[:32]

    with open('{}/{}/{}.md5'.format(export.config['export_dir'], export.config['exportpath_metaphlan'], OUTFILE_PREFIX), 'w') as tar_md5:
        tar_md5.write(calc_hash)
    
    with open('{}/{}/mpa_latest'.format(export.config['export_dir'], export.config['exportpath_metaphlan']), 'w') as latest:
    ...
    for s in species:
        ...
        try:
            marker_stat = pd.read_csv('{}/{}/markers_stats/{}.txt'.format(export.config['export_dir'], export.config['exportpath_metaphlan'], tax_id))
    ...
    all_markers_stats_df.to_csv('{}/{}/all_markers_stats.txt'.format(export.config['export_dir'], export.config['exportpath_metaphlan']), sep = '\t', index = False)
...
def build_virus_db(config, taxontree, proteomes):
    ...
    ncbi = NCBITaxa()
    new_db_fp = '{0}/{1}/{2}/{2}.pkl'.format(config['export_dir'], config['exportpath_metaphlan'], OUTFILE_PREFIX)
    ...
    mpa_v294_viruses_fp = '{0}/{1}/{2}_viruses/{2}.fna'.format(config['export_dir'], config['exportpath_metaphlan'], OUTFILE_PREFIX)
    mpa_v294_fp = '{0}/{1}/{2}/{2}.fna.bz2'.format(config['export_dir'], config['exportpath_metaphlan'], OUTFILE_PREFIX)
    ...
    with bz2.open('{0}/{1}/{2}_viruses/{2}.pkl'.format(config['export_dir'], config['exportpath_metaphlan'], OUTFILE_PREFIX), 'wb') as mpa_viruses_pkl:
