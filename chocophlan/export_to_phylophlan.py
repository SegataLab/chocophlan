...
class chocophlan2phylophlan:
    ...
    def run_export(self):
        ...
        if self.config['verbose']:
            utils.info('Started exporting CHOCOPhlAn data for PhyloPhlAn...')

        terminating = mpdummy.Event()
        with mpdummy.Pool(initializer = init_parse, initargs = (terminating, ), processes = self.config['nproc']) as pool:
        ...
        with open('{}/taxa2proteomes_cpa{}_up{}.txt'.format(self.exportpath,__CHOCOPhlAn_version__, __UniRef_version__), 'w') as t2p_out:
            with open('{}/taxa2core_cpa{}_up{}.txt'.format(self.exportpath,__CHOCOPhlAn_version__, __UniRef_version__), 'w') as t2c_out:
                with open('{}/taxa2genomes_cpa{}_up{}.txt'.format(self.exportpath,__CHOCOPhlAn_version__, __UniRef_version__), 'w') as t2g_out:
                    lines_t2p = ['#CHOCOPhlAn version {}\n'.format(__CHOCOPhlAn_version__), '#'+open('{}/_relnotes.txt'.format(self.config['download_base_dir'])).readline(), '#NCBI Taxonomy id\tFull Taxonomy\tList of proteomes\n']
                    lines_t2c = ['#CHOCOPhlAn version {}\n'.format(__CHOCOPhlAn_version__), '#'+open('{}/_relnotes.txt'.format(self.config['download_base_dir'])).readline(), '#NCBI Taxonomy id\tFull Taxonomy\tList of core proteins\n']
                    lines_t2g = ['#CHOCOPhlAn version {}\n'.format(__CHOCOPhlAn_version__), '#'+open('{}/_relnotes.txt'.format(self.config['download_base_dir'])).readline(), '#NCBI Taxonomy id\tFull Taxonomy\tList of genomes\n']
