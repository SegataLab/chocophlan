...
class export_to_metaphlan2:
    ...
    def get_uniref90_nucl_seq(self, item):
        ...
        with NamedTemporaryFile() as fna_decompressed:
            try:
    ...
    def get_uniref_uniprotkb_from_panproteome(self, panproteome):
        ...
        upid = ""
        res = []
        for marker in possible_markers.gene:
            try:
        ...
        try:
            with dummy.Pool(processes=3) as pool:
                res = [x for _ in pool.imap_unordered(self.get_uniref90_nucl_seq, gc.items(), chunksize=10) for x in _]
        except TypeError:
            ...
            if len(res):
                ...
                self.createCladeMarkers('export/metaphlan2/{}.fasta'.format(panproteome['tax_id']))

    def createCladeMarkers(self, marker):
        mpa_m = Fasta(marker)
        panproteome_id = marker.split('/')[-1].split('.')[0]
        panproteome = pickle.load(open('data/pickled/panproteomes/species/90/{}.pkl'.format(panproteome_id), 'rb'))
        clade = self.taxontree.taxid_n[panproteome['tax_id']]
        for seq in mpa_m:
            mpa_marker, nfid, taxonomy, gca = seq.long_name.split(' ')
            taxonomy = '{}|t__{}'.format(taxonomy, gca)
            seqs_dir = 'data/ncbi/{}/{}/'.format(gca[4:][:6], gca[4:][6:])
            in_seq_file = glob.glob(seqs_dir+'*fna*')[0]
            with NamedTemporaryFile() as fna_decompressed, gzip.open(in_seq_file, 'rb') as fna_gz, open(fna_decompressed.name, 'wb') as fna_out:
                shutil.copyfileobj(fna_gz, fna_out)
                fna = Fasta(fna_decompressed.name)
                # Add genome length for estimating coverage
                self.db['taxonomy'][taxonomy] = sum(record.rlen for record in fna.faidx.index.values())
                ...
                self.db['markers'][mpa_marker] = { 'clade': '{}__{}'.format(clade.rank[0], clade.name),
                                              'ext': [dict(self.proteomes[list(self.taxontree.taxid_n[x].proteomes)[0]]['ncbi_ids']).get('GCSetAcc','') for x in panproteome['members'][nfid.split('_')[1]]['external_species_nosp']['90_90']],
                                              'len': seq.unpadded_len,
                                              'score': 0,
                                              'taxon': taxonomy }
        os.unlink(marker+'.fai')
...
def run_all():
    ...
    with dummy.Pool(processes=20) as pool:
        failed = [x for x in pool.imap_unordered(export.get_uniref_uniprotkb_from_panproteome, species, chunksize=200)]
